...
server_port="`cat /etc/shadowsocks/config.json|grep -E '\"server\"'|awk -F ':' '{print $3}'`"
server_ips="`cat /etc/shadowsocks/config.json|grep -E '\"server\"'|awk -F ':' '{print $2}'|grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}'`"
while true;do
    for server_ip in $server_ips;do
        server_port="`cat /etc/shadowsocks/config.json|grep -E '\"server\"'|grep '$ip'|awk -F ':' '{print $3}'`"
        nc -v -z -w 1 $server_ip $server_port|grep "succeeded" -q
        if [ "$?" = 0 ];then
